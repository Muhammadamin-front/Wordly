...
export function Screen({ children, scroll = true, appHeader = false, appFooter = false, refreshing = false, onRefresh }: { children: ReactNode; scroll?: boolean; appHeader?: boolean; appFooter?: boolean; refreshing?: boolean; onRefresh?: () => void }) { const scrollRef = useRef<ScrollView>(null); const content = <View style={styles.content}>{children}</View>; const body = scroll ? <ScreenScrollContext.Provider value={scrollRef}><ScrollView ref={scrollRef} automaticallyAdjustKeyboardInsets keyboardDismissMode="interactive" contentContainerStyle={styles.scroll} refreshControl={onRefresh ? <RefreshControl refreshing={refreshing} onRefresh={onRefresh} tintColor={colors.rust} colors={[colors.rust]} /> : undefined} showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled">{content}</ScrollView></ScreenScrollContext.Provider> : content; return <SafeAreaView style={styles.safe} edges={appHeader ? [] : ["top"]}>{appHeader ? <AppHeader /> : null}<KeyboardAvoidingView behavior={Platform.OS === "ios" ? "padding" : undefined} style={styles.flex}>{body}</KeyboardAvoidingView>{appFooter ? <AppBottomNav /> : null}</SafeAreaView>; }
...
export function Heading({ children, sub }: { children: string; sub?: string }) { return <View style={styles.heading}><Text style={styles.h1}>{children}</Text>{sub ? <Text style={styles.sub}>{sub}</Text> : null}</View>; }
...
export function ErrorNote({ message }: { message?: string | null }) { return message ? <Text style={styles.error}>{message}</Text> : null; }
export function Loader({ label = "Loading" }: { label?: string }) { return <View style={styles.loader}><ActivityIndicator accessibilityLabel={label} color={colors.rust}/><Text style={styles.loaderText}>{label}</Text></View>; }
