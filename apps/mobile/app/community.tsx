...
function CommunityContent() {
  ...
  const myCode = useQuery({ queryKey: ["my-friend-code"], queryFn: () => request<Message>("/me/friend-code", { token }), enabled: !!token });
  ...
  return <Screen appHeader>
    ...
    <Paper style={styles.friendCode}>
      <View style={styles.friendCodeCopy}><Text style={styles.friendCodeLabel}>{t.yourCode}</Text><Text selectable style={styles.friendCodeValue}>{myCode.data?.message ?? "····"}</Text></View>
      <View style={styles.friendCodeActions}><Button variant="quiet" icon="person-outline" disabled={!myCode.data?.message} onPress={() => router.push(`/profile/${myCode.data?.message ?? ""}` as Href)}>{t.profile}</Button><Button variant="secondary" icon="share-outline" disabled={!myCode.data?.message} onPress={() => void Share.share({ message: myCode.data?.message ?? "" })}>{t.share}</Button></View>
    </Paper>
