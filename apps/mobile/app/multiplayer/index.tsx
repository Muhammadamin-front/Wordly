...
export default function Multiplayer() {
  ...
  const midGameDrop = Boolean(error) && phase !== "menu" && phase !== "connecting" && Boolean(code);

  return <Screen appHeader><View style={styles.hero}><Ionicons name="people-outline" size={29} color={colors.onAccent} /><Heading sub={t.subtitle}>{t.title}</Heading></View><ErrorNote message={error} />
    {midGameDrop ? <Button variant="secondary" icon="refresh-outline" onPress={() => code && join(code)}>{t.reconnect}</Button> : null}
