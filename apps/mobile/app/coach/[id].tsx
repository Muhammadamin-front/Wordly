...
export default function CoachChat() {
  ...
  const send = useMutation({ mutationFn: (text: string) => request<CoachTurn>(`/coach/sessions/${sessionId}/message`, { method: "POST", token, timeoutMs: 60_000, body: { text } }), onSuccess: (turn, text) => { setMessages((current) => [...current, { role: "user", content: text, corrections: [], created_at: new Date().toISOString() }, { role: "assistant", content: turn.reply, corrections: turn.corrections, created_at: new Date().toISOString() }]); setDraft(""); void queryClient.invalidateQueries({ queryKey: ["coach-session", sessionId] }); void queryClient.invalidateQueries({ queryKey: ["coach-dashboard"] }); } });
  const score = useMutation({ mutationFn: () => request<CoachScore>(`/coach/sessions/${sessionId}/score`, { method: "POST", token, timeoutMs: 60_000 }), onSuccess: () => void queryClient.invalidateQueries({ queryKey: ["coach-dashboard"] }) });
