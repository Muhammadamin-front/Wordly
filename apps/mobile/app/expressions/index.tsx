...
export default function ExpressionsScreen() {
  ...
  return (
    <Screen appHeader appFooter>
      ...
      <Modal visible={Boolean(openSlug)} animationType="slide" presentationStyle="pageSheet" onRequestClose={() => setOpenSlug(null)}><SafeAreaView edges={["top", "right", "bottom", "left"]} style={styles.modalSafe}>{detail.data ? <ExpressionModal locale={locale} expression={detail.data} added={added.has(detail.data.slug)} adding={addCard.isPending} canAdd={Boolean(token)} onAdd={() => addCard.mutate(detail.data)} onClose={() => setOpenSlug(null)} /> : detail.isError ? <View accessibilityRole="alert" style={styles.modalError}><Ionicons name="cloud-offline-outline" size={28} color={colors.rust} /><Text style={styles.errorText}>{t.error}</Text><Button icon="refresh" onPress={() => void detail.refetch()}>{t.retry}</Button><Button variant="quiet" onPress={() => setOpenSlug(null)}>{t.close}</Button></View> : <View accessibilityRole="progressbar" accessibilityLabel={t.title} style={styles.loader}><ActivityIndicator color={colors.rust} /><Text style={styles.loaderText}>{t.title}…</Text></View>}</SafeAreaView></Modal>
    </Screen>
  );
}
...
const styles = StyleSheet.create({
  ...
  searchBox: { minHeight: 50, flexDirection: "row", alignItems: "center", gap: 9, paddingHorizontal: 13, borderWidth: 1.5, borderColor: colors.line, borderRadius: 11, backgroundColor: colors.raised }, searchInput: { flex: 1, minHeight: 48, fontFamily: fonts.ui, fontSize: 16, color: colors.ink },
  filterRow: { gap: 7, paddingVertical: 2 }, filterChip: { minHeight: 48, justifyContent: "center", paddingHorizontal: 13, borderWidth: 1, borderColor: colors.line, borderRadius: 9, backgroundColor: colors.cream }, filterChipActive: { borderColor: colors.rust, backgroundColor: "rgba(185,78,40,0.10)" }, filterText: { fontFamily: fonts.uiBold, fontSize: 11, color: colors.muted }, filterTextActive: { color: colors.rustDark },
  loader: { minHeight: 180, alignItems: "center", justifyContent: "center", gap: 9 }, loaderText: { fontFamily: fonts.uiMedium, fontSize: 12, color: colors.muted }, errorCard: { gap: 10, padding: 15, borderWidth: 1, borderColor: "rgba(220,38,38,0.3)", borderRadius: 12 }, errorText: { fontFamily: fonts.uiMedium, fontSize: 13, lineHeight: 20, textAlign: "center", color: colors.danger }, modalError: { flex: 1, justifyContent: "center", gap: 14, paddingHorizontal: 24 },
