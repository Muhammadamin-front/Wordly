...
const LEVEL_ACCENT: Record<CefrGrammarLevel, string> = {
  ...
  A2: "border-success/50 bg-success/10 text-success-text",
  ...
  C1: "border-warning/50 bg-warning/10 text-warning-text",
};

const STATUS_STYLE: Record<GrammarMasteryStatus, string> = {
  "not-started": "border-line bg-card/70 text-ink-soft",
  weak: "border-danger/30 bg-danger/10 text-danger-text",
  "needs-review": "border-warning/30 bg-warning/10 text-warning-text",
  good: "border-brand-400/30 bg-brand-500/10 text-brand-600 dark:text-brand-300",
  mastered: "border-success/30 bg-success/10 text-success-text",
};
    ...
    <main id="main-content" tabIndex={-1} className="mx-auto w-full max-w-6xl flex-1 px-4 py-8 sm:px-6 sm:py-10">
      <section className="surface-panel light-sweep rounded-lg p-5 sm:p-8">
        <div className="grid gap-6 lg:grid-cols-[1.4fr_0.8fr] lg:items-end">
          <div>
            <span className="inline-flex items-center gap-2 rounded-lg border border-line bg-card/60 px-3 py-1.5 text-xs font-extrabold uppercase text-accent-text"><Layers3 className="size-4" aria-hidden />{t.studio}</span>
      ...
      <section className="mt-5 grid gap-3 lg:grid-cols-3">
        <label className="relative lg:col-span-2"><Search className="pointer-events-none absolute left-3 top-1/2 size-5 -translate-y-1/2 text-ink-soft" aria-hidden /><span className="sr-only">{copy.search}</span><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder={copy.search} className="min-h-12 w-full rounded-lg border border-line bg-card/80 py-3 pl-11 pr-4 text-base text-ink outline-none transition focus:border-brand-500 focus:ring-2 focus:ring-focus" /></label>
      ...
      <section className="mt-5 rounded-lg border border-warning/25 bg-warning/5 p-4 sm:p-5">
        <div className="flex items-start gap-3"><span className="icon-tile size-10 shrink-0 rounded-lg text-warning-text"><Target className="size-4" aria-hidden /></span><div className="min-w-0"><h2 className="font-extrabold text-ink">{copy.weak}</h2><p className="mt-0.5 text-sm text-ink-soft">{copy.weakHint}</p></div></div>
        {weak.length ? <div className="mt-3 grid gap-2 sm:grid-cols-3">{weak.map(({ lesson, score }) => <Link key={lesson.slug} href={`/${lang}/grammar/${lesson.slug}`} className="flex min-w-0 items-center justify-between gap-3 rounded-lg border border-line bg-card/70 px-3 py-2.5"><span className="truncate text-sm font-bold text-ink">{lesson.title}</span><span className="shrink-0 text-sm font-black text-warning-text">{score}%</span></Link>)}</div> : <p className="mt-3 text-sm text-ink-soft">{copy.noWeak}</p>}
      </section>
      ...
      <div className="mt-6 flex gap-2 overflow-x-auto pb-1" role="tablist" aria-label={t.cefrPath}>
        {GRAMMAR_LEVELS.map((item) => {
          ...
          if (locked) {
            return <Link key={item} href={`/${lang}/pricing`} role="tab" aria-selected={false} aria-label={`${item} — ${copy.premiumLocked}`} className="flex min-h-11 shrink-0 items-center gap-1.5 rounded-lg border border-line bg-card/40 px-4 py-2 text-sm font-extrabold text-ink-soft transition hover:bg-raised hover:text-ink active:translate-y-px">{item}<Lock className="size-3.5" aria-hidden /></Link>;
          }
          return <button key={item} type="button" role="tab" aria-selected={item === level} onClick={() => { setLevel(item); setCategory("all"); }} className={cn("min-h-11 shrink-0 rounded-lg border px-4 py-2 text-sm font-extrabold transition active:translate-y-px", item === level ? `${LEVEL_ACCENT[item]} shadow-lg` : "border-line bg-card/60 text-ink-soft hover:text-ink")}>{item}<span className="ml-1.5 text-xs font-bold">{mastered}/{levelItems.length}</span></button>;
        })}
      </div>

      <div className="mt-6 space-y-7">
        {grouped.length ? grouped.map(([group, items]) => items?.length ? <section key={group}>
          <div className="mb-3 flex items-end justify-between gap-3"><div><p className="text-xs font-extrabold uppercase tracking-wide text-accent-text">{level}</p><h2 className="text-xl font-extrabold text-ink">{group}</h2></div><span className="text-sm font-semibold text-ink-soft">{items.length}</span></div>
