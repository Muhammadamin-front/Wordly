...
# Redis holds rate-limit counters, cached corpus reads and live multiplayer
# rooms. It is only reachable from this host, but an unauthenticated instance
# on a shared Docker network is one misconfigured container away from being
# readable. Generated here rather than asked for, because a manual step in a
# deploy runbook is a step that never happens.
if ! grep -q "^REDIS_PASSWORD=." .env 2>/dev/null; then
  generated="$(head -c 32 /dev/urandom | od -An -tx1 | tr -d ' \n')"
  {
    echo ""
    echo "# Generated by deploy.sh. Redis is cache and ephemeral state only:"
    echo "# rotating this costs nothing but a container restart."
    echo "REDIS_PASSWORD=${generated}"
    echo "REDIS_URL=redis://:${generated}@redis:6379/0"
  } >> .env
  chmod 600 .env
  echo "Generated a Redis password and wrote it to .env"
else
...
# The tts_cache volume predates the non-root container and is owned by root
# on an existing host. Idempotent: after the first run this is a no-op.
docker compose --profile production run --rm --no-deps --user root api \
  chown -R 10001:10001 /srv/api/tts_cache
