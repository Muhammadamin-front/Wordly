...
function originalAcademicPassage(
  ...
  // slot, then reorder — so neither the composition nor the sequence repeats.
  const picked = rotated(
    [
      statements[variant % 2],
      statements[2 + ((variant >> 1) % 2)],
      statements[4 + ((variant >> 2) % 2)],
      statements[(variant >> 3) % 3 === 0 ? 1 - (variant % 2) : (variant >> 3) % 3 === 1 ? 3 - ((variant >> 1) % 2) : 5 - ((variant >> 2) % 2)],
    ],
    variant >> 4
  );
